(ns nonferrousmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the melting furnace, die-casting
  machine, or pouring line directly... does NOT authorize or execute
  maintenance/furnace operations') implemented faithfully. The single
  invariant under test:

    NonFerrousFoundryAdvisor never schedules maintenance, flags a
    safety concern, or coordinates a shipment the Non-Ferrous Foundry
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:alloy-grade :aluminum-silicon}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :aluminum-silicon (:alloy-grade (store/batch db "batch-001"))) "SSoT actually updated")
