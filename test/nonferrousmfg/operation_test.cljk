(ns nonferrousmfg.operation-test
  "Smoke tests for the compiled NonFerrousFoundryOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `nonferrousmfg.governor-contract-test`; the Store contract in
...
(deftest test-actor-builds
  (testing "NonFerrousFoundryOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "furnace-001" :severity :moderate :description "radiant heat rise near melting furnace"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
