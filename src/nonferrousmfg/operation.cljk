(ns nonferrousmfg.operation
  "NonFerrousFoundryOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (NonFerrousFoundryAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Non-Ferrous Foundry Plant Operations Governor (:govern) and the
  ...
  (:require [langgraph.graph :as g]
            ...
            [nonferrousmfg.advisor :as advisor]
...
(defn build
  "Compiles a NonFerrousFoundryOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `nonferrousmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; NonFerrousFoundryAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Non-Ferrous Foundry Plant Operations Governor -- independent censor.
      (g/add-node :govern
