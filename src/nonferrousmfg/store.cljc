(ns nonferrousmfg.store
  "SSoT for the non-ferrous metal casting foundry plant-operations
  ...
  (`cloud-itonami-isic-2431`'s own `foundrymfg.store`), this build
  ...
  self-contained (no external non-ferrous-metal-casting-foundry
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch
                             (a poured/die-cast heat/cast lot)'s
                             alloy-grade/weight/defect-rate record.
                             ...
                             on file in the foundry's production
                             ledger; `:shipped-weight-kg` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a melting-furnace/mold-station/die-casting-
                             machine/shakeout-line unit's own record.
    ...
    - `maintenance`       -- a scheduled furnace/mold/shakeout/die-
                             casting-equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`nonferrousmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound non-ferrous casting
                             shipment DRAFT (`nonferrousmfg.registry`'s
                             `register-shipment`).
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :alloy-grade :aluminum-silicon :output-form :die-cast
                :material "Aluminum-Silicon Automotive Housing Castings"
                :weight-kg 20000.0 :defect-rate-percent 1.0
                :verified? true :registered? true
                :shipped-weight-kg 4000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :alloy-grade :bronze :output-form :sand-cast
                :material "Bronze Marine Propeller Castings"
                :weight-kg 3000.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-weight-kg 2800.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :alloy-grade :zinc-aluminum :output-form :die-cast
                :material "Zinc-Aluminum Hardware Castings"
                :weight-kg 12000.0 :defect-rate-percent 1.8
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"furnace-001" {:id "furnace-001" :kind :reverberatory-furnace
                  :verified? true :registered? true
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `nonferrousmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `nonferrousmfg.registry` and
...
(defn mem-store
  ...
  (->MemStore (atom {:batches {} :equipment {} :maintenance {} :shipments {}
                      :dust-controls {} :records {} :safety-concerns []
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered reverberatory-
  furnace unit (schedulable for maintenance), one UNVERIFIED/
  unregistered die-casting-machine unit (blocks any maintenance
